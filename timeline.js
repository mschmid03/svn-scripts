...
const MotionPathPlugin_1 = require("gsap/MotionPathPlugin");
const ScrollTrigger_1 = require("gsap/ScrollTrigger");
/* The following plugin is a Club GSAP perk */
const DrawSVGPlugin_1 = require("gsap/DrawSVGPlugin");
gsap_1.gsap.registerPlugin(ScrollTrigger_1.ScrollTrigger, MotionPathPlugin_1.MotionPathPlugin, DrawSVGPlugin_1.DrawSVGPlugin);
// commit comment
let tl = gsap_1.gsap.timeline({
...
let mm = gsap_1.gsap.matchMedia();
// We already have a full-duration animation from clear/to above
// Base animation for both desktop and mobile
tl.fromTo(".timeline_line-svg.is-animated", // Removed tablet-up qualifier to apply to both
{ drawSVG: "0% 0%" }, { duration: 3500, drawSVG: "0% 100%" }, "one");
// Desktop-specific animations
mm.add("(min-width: 768px)", () => {
    // console.log("Desktop view active");
    tl.to("#timeline-dates-container", {
        ease: "none",
        duration: 3500,
        top: "-80.5rem",
    }, "one");
    tl.fromTo(".timeline_heading", {
        autoAlpha: 1,
    }, {
        ease: "none",
        duration: 150,
    ...
    const blobSVGs = gsap_1.gsap.utils.toArray("#desktop-dates .timeline_blob-svg");
    blobSVGs.splice(0, 1);
    tl.fromTo(blobSVGs, { opacity: "60%" }, { duration: "150", opacity: "100%", stagger: { each: 500 } }, "two-=200");
});
// Mobile-specific animations
mm.add("(max-width: 767px)", () => {
    // console.log("Mobile view active");
    tl.to("#timeline-dates-container-s", {
        ease: "none",
        duration: 3500,
        left: "-80.5rem",
    }, "one");
    const blobSVGs = gsap_1.gsap.utils.toArray("#mobile-dates .timeline_blob-svg");
    blobSVGs.splice(0, 1);
    tl.fromTo(blobSVGs, { opacity: "60%" }, { duration: "150", opacity: "100%", stagger: { each: 500 } }, "two-=200");
});
// Get all background images without modifying the array
const bgImages = gsap_1.gsap.utils.toArray(".timeline_background-image");
const allBgImages = [...bgImages]; // Create a copy to avoid mutation issues
// First image animations
tl.fromTo(allBgImages[0], { autoAlpha: 1 }, { autoAlpha: 0, duration: 150 }, "one+=175");
tl.fromTo(allBgImages[0], { top: "-7.5%" }, { top: "-15%", duration: 325 }, "one");
// Middle images animations (2-7)
const middleImages = allBgImages.slice(1, 7);
tl.fromTo(middleImages, { autoAlpha: 0 }, {
    autoAlpha: 1,
    duration: 150,
    stagger: {
        each: 500,
...
tl.fromTo(allBgImages[7], // Assuming there are 8 images total
{ top: "0%" }, { top: "-7.5%", duration: 325 }, "seven+=175");
// Use position parameter string with label to ensure animations extend to the end
tl.fromTo(middleImages, { autoAlpha: 1 }, {
    autoAlpha: 0,
    duration: 150,
    stagger: {
        each: 500,
...
ScrollTrigger_1.ScrollTrigger.refresh(true);
